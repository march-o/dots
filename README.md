...
cp -r ./* ~/.hyprconfig
# Rename config file
mv ~/.hyprconfig/_hyprland.conf ~/.hyprconfig/hyprland.conf 
```

### Switching Primary Screen
Add this line to your config:
```conf
exec=~/.hyprconfig/scripts/variables/set_env primary [ID_OF_PRIMAR_SCREEN] # 0, 1, 2, ...
```
...
#### Webcord
```bash
webcord --add-css-theme ~/.hyprconfig/dots/BetterDiscord/Apatheia.theme.css
```
...
example:
```bash
ls /home/amadeusw/.hyprconfig/themes/apatheia/wallpapers | wofi --show dmenu
```
